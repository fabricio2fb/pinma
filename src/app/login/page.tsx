...
export default function LoginPage() {
  ...
  const Form = ({ isRegister = false }: { isRegister?: boolean }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        // Import das ações no topo do script (lazy para Client Component)
        ...
        if (err.message && err.message.includes('NEXT_REDIRECT')) {
          throw err; // Allow Next.js redirect to happen
        }
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        {errorMsg && (
          <div className="p-3 text-sm bg-destructive/15 text-destructive border border-destructive/30 rounded-md">
            {errorMsg}
          </div>
        )}
        ...
        {isRegister && (
          <div className="relative">
            ...
            <Input name="username" type="text" placeholder="Nome de usuário (@seunome)" className="pl-12 bg-muted focus-visible:ring-offset-0" required />
          </div>
        )}
        <div className="relative">
          {isRegister ? (
          ...
          <Input
            ...
            className="pl-12 bg-muted focus-visible:ring-offset-0"
            required
          />
        </div>
        <div className="relative">
          <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
          <Input name="password" type={showPassword ? 'text' : 'password'} placeholder="Senha" className="pl-12 pr-12 bg-muted focus-visible:ring-offset-0" required />
          <Button
            type="button"
            variant="ghost"
            size="icon"
        ...
        {isRegister && (
          <div className="relative">
            <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
            <Input type={showPassword ? 'text' : 'password'} placeholder="Confirmar Senha" className="pl-12 pr-12 bg-muted focus-visible:ring-offset-0" required />
          </div>
        )}
