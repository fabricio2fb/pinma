...
// ─── Busca Overpass ───────────────────────────────────────
async function buscarEstabelecimentos(lat: number, lng: number, raio = 3000): Promise<OverpassPlace[]> {
  // Puxa TUDO de uma vez com union ampla
  const query = `
[out:json][timeout:30];
(
  ...
  node["office"](around:${raio},${lat},${lng});
);
out body;`;
...
function getCategoryInfo(tags: Record<string, string>) {
  for (const cat of OVERPASS_FILTERS) {
    const cleanQuery = cat.query.replace(/["\[\]]/g, '');
    const equalIndex = cleanQuery.indexOf('=');
    const key = cleanQuery.substring(0, equalIndex);
    const value = cleanQuery.substring(equalIndex + 1);
    if (tags[key] === value) return cat;
  }
  // Fallback — bolinha cinza pra qualquer coisa não mapeada
  return { label: tags.name || tags.amenity || tags.shop || tags.tourism || tags.leisure || 'Local', color: '#6b7280' };
}
